			...
			<div class="main-top">
				<div class="wrapper padding-bord">
					...
					<%
						...
						ResultSet rs = DatabaseConnection.getResultFromSqlQuery("select * from tblnotice where id='" + noticeId + "'");
						while (rs.next()) {
					%>
					<form action="UpdateNotice" method="post">
						<div class="d-grid grid-col-1">
							<div class="form-group">
								<input type="text" name="noticeId" class="form-control" value="<%=rs.getInt("id") %>" required hidden/>
							</div>
						</div>
						<div class="d-grid grid-col-1">
							<div class="form-group">
								<p class="text-head">Notice Title</p>
								<input type="text" name="ntitle" class="form-control" value="<%=rs.getString("title") %>" required />
							</div>
						</div>
						<div class="d-grid grid-col-1">
							<div class="form-group">
								...
								<textarea name="description" class="form-control" required
									cols="142" rows="10" value="<%=rs.getString("info") %>"><%=rs.getString("info") %></textarea>
							</div>
						</div>
						<button type="submit" class="buttonbg signinbutton">Update
							Notice</button>
					</form>
					<%
						}
